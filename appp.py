...
IS_RPI=IS_LINUX and os.path.exists("/sys/firmware/devicetree/base/model")
# ==========================================================
# ENV + BASIC SETUP
# ==========================================================
load_dotenv()
...
def update_config_cache():
    ...
    for row in rows:
        ...
        if key in ["email_recipients", "cam_resolution", "cam_quality", "ppe_class_configs"]:
            mem[key] = str(value)
        elif "thresh" in key or key in ["cam_zoom", "cam_brightness", "cam_fps"]:
            try:
                mem[key] = float(value)
            except ValueError:
                mem[key] = value
...
def setup_gpio():
    if not IS_RPI:
        logging.info("GPIO skipped: non-Raspberry Pi system")
        return
...
def setup_serial():
    ...
    logging.info("Serial skipped/not found")
...
def generate_frames():
    while True:
        ...
        conf = get_config()
        quality = conf.get("cam_quality", "SD")
